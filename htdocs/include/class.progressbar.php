...
class ProgressBar
{
	...
	*/
	...
	function getHtml()
	{
		...
		$style_brd = 'position:relative;top:'.$this->top.'px;left:'.$this->left.'px;width:'.$this->width.'px;height:'.$this->height.'px;background:'.$this->bgr_color.';';
		if ($this->border>0) {$style_brd .= 'border:'.$this->border.'px solid;border-color:'.$this->brd_color.';';}
		
		$style_bar = 'position:relative;top:'.$this->position['top'].'px;left:'.$this->position['left'].'px;'.'width:'.$this->position['width'].'px;height:'.$this->position['height'].'px;background:'.$this->color.';';
		
		if ($this->frame['show']==true) {
			if ($this->frame['border']>0) {$border = 'border:'.$this->frame['border'].'px solid;border-color:'.$this->frame['brd_color'].';';}
			$html = '<div id="pfrm'.$this->code.'" style="position:relative;top:'.$this->frame['top'].'px;left:'.$this->frame['left'].'px;width:'.$this->frame['width'].'px;height:'.$this->frame['height'].'px;'.$border.'background:'.$this->frame['color'].';">'."\n";
		}
		...
		foreach($this->label as $name => $data) {
			$style_lbl = 'position:relative;top:'.$data['top'].'px;left:'.$data['left'].'px;text-align:'.$data['align'].';';
